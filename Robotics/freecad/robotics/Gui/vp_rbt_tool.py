...
from freecad.robotics.App.rbt_helpers_log import fcl_err, fcl_msg, fcl_warn
from freecad.robotics.App.rbt_helpers_frames import jog_rotation
from freecad.robotics.App.rbt_tool import tool_parent
from freecad.robotics.Gui.taskpanel_rbt_tool import DefineTCP
...
class ViewProviderTool:
    ...
    def _on_sphere_motion(self, userdata=None, event_cb=None):
        """
        Callback for dragging motion over the TCP Sphere
        """
        if (not self.f_dragging) or (event_cb is None):
            return
