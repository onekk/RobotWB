...
from freecad.robotics.App.rbt_placement import p_asm_in_world, base_link
from freecad.robotics.App.rbt_helpers_math import deg_to_rad, rad_to_deg
...
def joint_value_doc(j, d, z=0.0):
    ...
    if jt == REVOLUTE:
        diff = float(j.Offset2.Rotation.toEuler()[0]) - z
        return d * (((diff + 180.0) % 360.0) - 180.0)
    if jt == PRISMATIC:
